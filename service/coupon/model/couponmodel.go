...
type (
	CouponModel interface {
		...
		GetCouponList(in *coupon.CouponListReq) ([]*Coupon, int64, error)
	...
	Coupon struct {
		gorm.Model
		Type         int64     `json:"type" gorm:"type:bigint;comment:优惠券类型;not null"`       //优惠券类型；0->全场赠券；1->会员赠券；2->购物赠券；3->注册赠券'
		Name         string    `json:"name" gorm:"type:varchar(191);comment:名称;not null"`    //名称
		Platform     int64     `json:"platform" gorm:"type:bigint;comment:使用平台;not null"`    //使用平台
		...
		UseType      int64     `json:"use_type" gorm:"type:bigint;comment:使用类型"`             //使用类型：0->全场通用；1->指定分类；2->指定商品
...
//
////GetUserList 获取用户列表
func (m *defaultCouponModel) GetCouponList(in *coupon.CouponListReq) ([]*Coupon, int64, error) {
	var list []*Coupon
	...
	if in.Platform != 0 {
		db = db.Where("email LIKE ?", fmt.Sprintf("%%%s%%", in.Platform))
	}
	if in.StartTime != "" {
	...
	if in.UseType != 0 {
		db = db.Where("user_type = ?", fmt.Sprintf("%%%s%%", in.UseType))
	}
	var total int64
