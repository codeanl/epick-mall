...
type (
	LoginLogModel interface {
		...
		GetLoginLogList(in *system.LoginLogListReq) ([]*LoginLog, int64, error)
...
func (m *defaultLoginLogModel) GetLoginLogList(in *system.LoginLogListReq) ([]*LoginLog, int64, error) {
	var list []*LoginLog
	db := m.conn.Model(&LoginLog{}).Order("created_at DESC")
	if in.Phone != "" {
		db = db.Where("nickname LIKE ?", fmt.Sprintf("%%%s%%", in.Phone))
	}
	if in.Ip != "" {
